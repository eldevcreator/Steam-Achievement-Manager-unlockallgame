...
namespace SAM.Game
{
    internal partial class Manager : Form
    {
        ...
        private void OnUnlockAllGames(object sender, EventArgs e)
        {
            ...
            int appCount = this._SteamClient.SteamApps008.GetInstalledApps(null, 0);
            if (appCount > 0)
            {
                uint[] appIds = new uint[appCount];
                this._SteamClient.SteamApps008.GetInstalledApps(appIds, appCount);
                games.AddRange(appIds);
            }
            ...
            // Отключаем интерфейс
            this.DisableInput();
            this._GameStatusLabel.Text = $"Processing {games.Count} games...";
            ...
            {
                processedGames++;
                this._GameStatusLabel.Text = $"Processing game {processedGames}/{games.Count} (AppID: {appId})...";
                Application.DoEvents();

                try
                {
                    // Загружаем схему для игры
                    ...
                    // Получаем список достижений из схемы
                    ...
                    if (stats.Valid && stats.Children != null)
                    {
                        foreach (var stat in stats.Children)
                        {
                            ...
                            var typeNode = stat["type"];
                            string typeStr = typeNode.Valid && typeNode.Type == KeyValueType.String
                                ? (string)typeNode.Value
                                : null;

                            if (typeStr == "4" || typeStr == "5" || 
                                typeStr?.ToLower() == "achievements" || 
                                typeStr?.ToLower() == "groupachievements")
                            {
                                if (stat.Children != null)
                                {
                                    foreach (var bits in stat.Children.Where(
                                        b => string.Compare(b.Name, "bits", StringComparison.InvariantCultureIgnoreCase) == 0))
                                    {
                                        if (bits.Valid && bits.Children != null)
                                        {
                                            foreach (var bit in bits.Children)
                                            {
                                                string achId = bit["name"].AsString("");
                    ...
                    {
                        continue; // Нет достижений - пропускаем
                    }

                    // Запрашиваем статистику для игры
                    var steamId = this._SteamClient.SteamUser.GetSteamId();
                    var callHandle = this._SteamClient.SteamUserStats.RequestUserStats(steamId);
                    ...
                    {
                        if (this._SteamClient.SteamUserStats.SetAchievement(achId, true))
                        {
                            totalAchievements++;
                        }
                    }
            ...
            this.EnableInput();
            this._GameStatusLabel.Text = $"Completed! Processed {processedGames} games, unlocked {totalAchievements} achievements.";

            MessageBox.Show(
                this,
                $"Completed!\n\nProcessed {processedGames} games\nUnlocked {totalAchievements} achievements",
                "Success",
                MessageBoxButtons.OK,
